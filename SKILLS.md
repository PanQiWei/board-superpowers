...
| `consuming-card` | Molecular | **v1-minimum** | Consumer surface — Shape X mega-routine hosting all 23 journey nodes (F1-F4 stages + B1-B5 bootstrap + G1-G4 governance + C1-C4 composing-siblings handoffs); delivers each card's full lifecycle from claim through PR merge cleanup. Refactored in v0.6.0 to the 23-node encoding. |
...
- **Called by**: every molecular skill that delegates to sibling
  plugins — today: `managing-board` (intake routing to
  `gstack:/*` / `superpowers:*`), `consuming-card` (B1 plan

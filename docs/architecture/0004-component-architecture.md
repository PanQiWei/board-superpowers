...
| Producer daily/intake/review/triage routines | skills (`briefing-daily` / `intaking-requirement` / `reviewing-pr-queue` / `triaging-board`) | per-routine reference files | Model-driven workflows reading the board via `gh` and recommending action. Each routine is a dedicated SKILL with its own trigger vocabulary (Shape Y per `03-producer-surface-redesign.md`). |

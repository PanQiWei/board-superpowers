...
| `0002-product-features-and-flows/08-pr-contract.md` (three-section shape) | `consuming-card` skill spec (Stage 4 D1 PR submit); `enforcing-pr-contract` skill spec; `managing-board` Review Queue routine spec (F-02 violation flagging). |
...
| `post_merge_cleanup` config field (auto-cron tracking) | `0005-contracts/03-config-schemas.md` § post_merge_cleanup; `0002-product-features-and-flows/04-consumer-surface.md` (Stage 4 submit + E1 termination nodes); `../../skills/consuming-card/SKILL.md` Stage 4 pre-flight + E1 post-merge cleanup; `../../skills/consuming-card/references/post-merge-cleanup.md`; `../../scripts/post-merge-cleanup.sh` + `../../scripts/install-post-merge-cron.sh`. |

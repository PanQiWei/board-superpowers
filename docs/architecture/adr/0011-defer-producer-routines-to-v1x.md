...
*Reason:* For F-11 (Stale session detection): triage cadence presupposes multi-day idle cards as the typical case, and AI-cadence cards rarely sit longer than hours; on the rare occasion they do, the architect notices through F-02's Review Queue, so the dedicated detection routine has not earned its keep in dogfood operation. For F-10 (Triage with remediation ladder): the ladder mechanic itself has standalone value (structured prescriptions for "block in In Review for N hours / Consumer death / dependency disappeared"), but the structure cost only amortizes once triage volume crosses a threshold the AI-cadence single-architect operating mode has not reached; the ad-hoc decision the architect makes today is cheaper than the ladder's structure overhead. Both reopen on observed demand.

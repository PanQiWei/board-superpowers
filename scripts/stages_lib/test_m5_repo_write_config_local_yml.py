"""Tests for stages_lib.m5_repo_write_config_local_yml.
...
# ---------------------------------------------------------------------------
from stages_lib.m5_repo_write_config_local_yml import (
...
def test_compute_target_state_path_value(ctx):
    state = compute_target_state(ctx)
    expected = str(ctx.repo_root / ".board-superpowers" / "settings.local.yml")
    assert state["path"] == expected


def test_compute_target_state_path_is_local_yml(ctx):
    """settings.local.yml not settings.yml — gitignored per M6 stage."""
    state = compute_target_state(ctx)
    assert "settings.local.yml" in state["path"]
...
def test_idempotency_check_current_state_has_schema_version(ctx):
    executor(ctx)
    result = idempotency_check(ctx)
...
def test_idempotency_check_wrong_schema_version(ctx):
    ...
    path = bsp_dir / "settings.local.yml"
...
# ---------------------------------------------------------------------------
# executor() — creates settings.local.yml
# ---------------------------------------------------------------------------
...
def test_executor_message_contains_local_yml(ctx):
    result = executor(ctx)
    assert "settings.local.yml" in result["message"]
...
def test_compute_target_state_validates_against_registry_schema(ctx):
    ...
    stage = next(
        ...
        if s["stage_id"] == "m5.repo.write-config-local-yml"
    )

"""Tests for stages_lib.m7_repo_inject_block_skill_routing — skill-routing injection.
...
two-block coexistence (routing-rule + skill-routing), and schema round-trip.
...
from stages_lib.m7_repo_inject_block_skill_routing import (
    _BLOCK_CLOSE,
    _BLOCK_NAME,
    _BLOCK_OPEN,
...
def _seed_detect_cache(ctx: SimpleNamespace, form: str) -> None:
    ...
    agentsmd_present = form in ("codex-only", "dual")
    claudemd_present = form in ("cc-only", "dual")
    _ps.update_module_section(
        ...
        "m7_agent_routing",
        {
            "agentsmd_form": {
                "form": form,
                "agentsmd_present": agentsmd_present,
                "claudemd_present": claudemd_present,
            }
        },
        **kwargs,
    )
...
def test_compute_target_state_block_name():
    """compute_target_state must return block_name='skill-routing'."""
    with tempfile.TemporaryDirectory() as h, tempfile.TemporaryDirectory() as r:
        ...
        assert state["block_name"] == "skill-routing"
...
def test_compute_target_state_block_present_empty_initially():
    ...
    with tempfile.TemporaryDirectory() as h, tempfile.TemporaryDirectory() as r:
        (Path(r) / "AGENTS.md").write_text("# No block yet\n")
        ctx = make_ctx(h, r)
        _seed_detect_cache(ctx, "codex-only")
        state = compute_target_state(ctx)
...
def test_predicate_accepts_valid_state():
    state = {"block_name": "skill-routing", "block_present_in_targets": ["AGENTS.md"]}
    assert target_state_predicate(state) is True


def test_predicate_accepts_empty_targets():
    state = {"block_name": "skill-routing", "block_present_in_targets": []}
    assert target_state_predicate(state) is True
...
def test_predicate_rejects_non_dict():
    assert target_state_predicate(42) is False
...
def test_executor_appends_block_when_absent():
    ...
    with tempfile.TemporaryDirectory() as h, tempfile.TemporaryDirectory() as r:
        ...
        assert _SKILL_ROUTING_CONTENT in text
...
def test_executor_replaces_drifted_block():
    """executor must replace a managed block whose content drifted."""
    with tempfile.TemporaryDirectory() as h, tempfile.TemporaryDirectory() as r:
        agents = Path(r) / "AGENTS.md"
        agents.write_text(
            f"# AGENTS\n\n{_BLOCK_OPEN}\nold stale content\n{_BLOCK_CLOSE}\n\n# After section\n"
...
# ---------------------------------------------------------------------------
# Two-block coexistence — routing-rule + skill-routing can coexist
# ---------------------------------------------------------------------------


def test_both_blocks_coexist_in_agents_md():
    """routing-rule and skill-routing blocks must be injectable independently."""
    from stages_lib.m7_repo_inject_block_routing_rule import (
        _BLOCK_OPEN as RR_OPEN,
        executor as rr_executor,
    )
    ...
    with tempfile.TemporaryDirectory() as h, tempfile.TemporaryDirectory() as r:
        ...
        # Inject routing-rule first
        rr_executor(ctx)
        # Then inject skill-routing
        executor(ctx)

        text = (Path(r) / "AGENTS.md").read_text()
        assert RR_OPEN in text
        assert _BLOCK_OPEN in text
        assert "Original content." in text


def test_skill_routing_inject_does_not_disturb_routing_rule():
    """Injecting skill-routing must not corrupt the routing-rule block."""
    from stages_lib.m7_repo_inject_block_routing_rule import (
        _ROUTING_RULE_CONTENT as RR_CONTENT,
        executor as rr_executor,
    )
...
def test_executor_skips_stub_redirect_claude_md():
    """executor must skip CLAUDE.md when it is a stub-redirect."""
    with tempfile.TemporaryDirectory() as h, tempfile.TemporaryDirectory() as r:
        (Path(r) / "AGENTS.md").write_text("# AGENTS\n")
...
def test_schema_roundtrip():
    ...
    schema = _get_registry_schema("m7.repo.inject-block.skill-routing")
    with tempfile.TemporaryDirectory() as h, tempfile.TemporaryDirectory() as r:
        (Path(r) / "AGENTS.md").write_text("# AGENTS\n")
        ctx = make_ctx(h, r)
        _seed_detect_cache(ctx, "codex-only")

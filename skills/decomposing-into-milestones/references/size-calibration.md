...
board-superpowers does not track these metrics automatically (no KPI infra by design). The architect surfaces drift via retro routines (`weekly-retro` in `managing-board`'s F-13). Manual + low-cadence is sufficient; no dashboard needed.

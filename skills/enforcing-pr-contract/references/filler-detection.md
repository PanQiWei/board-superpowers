...
Edits **inside** either marker pair (other than re-emitting the exact output of `bsp_render_creator_trace_block` or the canonical audit-trail content) are rejected at PR-submit time. Specifically:
...
The PR-submit validator's regex enforcement is in `scripts/submit-pr.sh` (Contract A + B section). When this card lands, also extend that regex to recognize the new marker pair (this is a separate enforcement-side change tracked in chunks 9-10 of this PR).

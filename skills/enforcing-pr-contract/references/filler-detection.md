...
Hand-edit rejection is currently doc-only — there is no automated
gate that rejects edits inside the marker pair at PR-submit time
or commit time. The intent is to wire enforcement into the
action_id 112 (PR-submit pre-flight card body sync) path in a
future card. Until then, reviewers should manually flag any card
body PR that mutates marker contents away from
`bsp_render_creator_trace_block` output (for `creator-trace`) or
the canonical audit-trail content.

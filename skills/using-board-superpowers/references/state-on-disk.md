...
| `~/.board-superpowers/repos/<normalized>/state.yml` | host-local | `bootstrap-project.sh` (initial write), `bsp_inject_routing_block` (hash updates), `bootstrapping-repo` (`last_seen_version_in_repo` bumps as part of the unified setup-stages flow per ADR-0012) | SessionStart hook, entry skill, tamper-detection routines | Per-`(host, repo)` durable state: `schema_version`, `repo_bootstrapped_at`, `last_seen_version_in_repo`, `features_enabled` list, `routing_blocks[].block_hash` for tamper detection. **Does NOT carry `audit_db_url`** — that lives in `credentials.yml` below. |

#!/usr/bin/env bash
# tests/post-merge-cleanup-pr-closed.sh — assert post-merge-cleanup.sh
# exits 3 when the PR is CLOSED without merge (failure path; action_id 103).
#
# Fake gh PATH-shim returns a CLOSED (not merged) JSON payload.
...
# ---------------------------------------------------------------------------
# Build fake gh shim returning CLOSED (no merge)
# ---------------------------------------------------------------------------
...
case "${1:-}" in
    pr)
        ...
        case "${1:-}" in
            list)
                # CLOSED without merge: mergedAt is null, state is CLOSED.
                printf '{"number":77,"state":"CLOSED","mergedAt":null,"headRefName":"claim/99-some-title"}\n'
                exit 0
                ;;
        esac
        ;;
...
printf '\nRunning post-merge-cleanup.sh with CLOSED (no-merge) PR ...\n'
...
check "exit code 3 (CLOSED without merge — failure path / action_id 103)" test "${RC}" -eq 3

# Worktree must be intact (we only clean up on MERGED).
check "worktree still present (not removed on CLOSED)" test -d "${WT_PATH}"
